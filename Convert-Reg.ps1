	  ...
	  ForEach ($RegValue in $RegValues)
		{
		  ...
		  If ($RegValue -notmatch '^\"')
			  { Write-Host "[ERROR] BINARY/QWORD/MULTI_SZ/EXPAND_SZ values are currently not supported. Exiting..." -ForegroundColor Red; Exit }
		  # CHECK IF BINARY VALUE
		  ElseIf ($RegValue -match "\=hex\:")
			  { Write-Host "[ERROR] BINARY values are currently not supported. Exiting..." -ForegroundColor Red; Exit }
		  # CHECK IF QWORD VALUE
		  ElseIf ($RegValue -match "\=hex\(b\)")
			  { Write-Host "[ERROR] QWORD values are currently not supported. Exiting..." -ForegroundColor Red; Exit }
		  # CHECK IF MULTI STRING VALUE
		  ElseIf ($RegValue -match "\=hex\(7\)")
			  { Write-Host "[ERROR] MULTI_SZ values are currently not supported. Exiting..." -ForegroundColor Red; Exit }
		  # CHECK IF EXPAND STRING VALUE
		  ElseIf ($RegValue -match "\=hex\(2\)")
			  { Write-Host "[ERROR] EXPAND_SZ values are currently not supported. Exiting..." -ForegroundColor Red; Exit }
